 ...
 * 
 * Contributor(s): Mike Ang Igor Bukanov Yuh-Ruey Chen Ethan Hugg Bob Jervis
 * Terry Lucas Mike McCabe Milen Nankov Norris Boyd
 * 
...
import org.fireinsight.proxy.InsightProxyMain;
...
public class Parser
{
    /* Script Insight property name */
    private static final String SCRIPT_PROP = "Script";
    
    /* Script Insight Instrumentation code to add */
    private static final String SCRIPT_INSIGHT_ANALYSIS_CODE = Parser.getInstrumentationCode();

    private String[] fields = { "height" };
    private List<String> vec = new ArrayList<String>();
    ...
    protected Decompiler createDecompiler(CompilerEnvirons compilerEnv)
    {
        ...
        /* Inject Script Insight code */
        decompiler.addName(Parser.SCRIPT_INSIGHT_ANALYSIS_CODE);
        return decompiler;
    }
            ...
            int lineno)
    {
        ...
        this.sourceURI = parts[parts.length - 1];
     ...
     */
    private Node parseFunctionBody(String name) throws IOException
    {
        ++nestingOfFunction;
        Node pn = nf.createBlock(ts.getLineno());
        if (name != null && name.trim().length() > 0) {
            decompiler.addName("document.pushFunctionInfo('" + name + "', '" + sourceURI + "', " + ts.getLineno() + ");");
        }
        else {
            decompiler.addName("document.pushFunctionInfo('{anonymous}', '" + sourceURI + "', " + ts.getLineno() + ");");
        }
        try {
            bodyLoop: for (;;) {
                Node n;
    ...
    private Node function(int functionType) throws IOException, ParserException
    {
        ...
        if (matchToken(Token.NAME)) {
            ...
            // Anonymous function
            name = "";
            //decompiler.addName("/* ANONYMOUSSTART " + (counter) + ": (" + baseLineno + ") */");
        }
        else {
            name = "";
            if (compilerEnv.isAllowMemberExprAsFunctionName()) {
        ...
        try {
            ...
            decompiler.addName(" /* FUNCTIONRETURN 3 (NO RETURN) */ document.popFunctionInfo();\n /* alert('NO RETURN'); */");
        ...
        //System.out.println("Source URI: " + sourceURI + "\nStart Line: " + baseLineno + "\nEnd Line: " + ts.getLineno());
        //decompiler.addName("/* ANONYMOUS END Source URI: " + sourceURI + "\nStart Line: " + baseLineno + "\nEnd Line: " + ts.getLineno() + " */");
        //decompiler.addName("document.pushFunctionInfo('" + sourceURI + "', '" + baseLineno + "', '" + ts.getLineno() + "');");
    ...
    private Node statement() throws IOException
    {
        ...
        Node n = nf.createExprStatement(nf.createName("error"), lineno);
        return n;
    }

    private void handlePN(Node pn)
    {
        int type = pn.getType();
        switch (type) {
        case Token.VAR: {
            // decompiler.addName("alert("+pn.getFirstChild().getString()+");");
            break;
        }
        case Token.EXPR_VOID: {

            myLine = pn.getLineno();
            if (pn.getFirstChild().getType() == Token.SETNAME) {

            }
            else if (pn.getFirstChild().getType() == Token.SETPROP) {

                // decompiler.addName("alert("+pn.getFirstChild().getFirstChild().getString()+");");
                String str = print(pn.getFirstChild().getFirstChild());

                Node fieldNode = pn.getFirstChild().getFirstChild().next;
                if (fieldNode == null) {
                    System.out.println();
                }
                // if (vec.contains(fieldNode.getString()))
                if (true) {
                    if (str.endsWith("style")) {
                        decompiler.addName(" document.appendInfo0("
                                + str.replace(".style", "") + ", '____style."
                                + fieldNode.getString() + "', '"
                                + pn.getLineno() + "@" + this.sourceURI
                                + "'); ");
                    }
                    else {
                        decompiler.addName(" document.appendInfo0(" + str
                                + ", '____" + fieldNode.getString() + "', '"
                                + pn.getLineno() + "@" + this.sourceURI
                                + "'); ");
                    }
                }
            ...
            /*
             * case Token.EOF: case Token.EOL: case Token.SEMI: break
    ...
    private String print(Node firstChild)
    {
        ...
        case Token.SETPROP: {
            Node first = firstChild.getFirstChild();
            Node second = first.next;
            Node third = second.next;
            return print(first) + "." + print(second) + " = " + print(third);
        }
        case Token.GETPROP: {
            Node first = firstChild.getFirstChild();
            Node second = first.next;
            return print(first) + "." + second.getString();
        }
        case Token.GETELEM: {
            Node first = firstChild.getFirstChild();
            Node second = first.next;
            return print(first) + "[" + print(second) + "]";
        }
        ...
        case Token.NEW: {
            ...
            return "new " + print(first) + "(" + args + ")";
        }
        default: {
            throw new RuntimeException(firstChild.getType() + "");
        }
            ...
            ParserException
    {
        ...
        case Token.VAR: {
            popFlag = true;
            decompiler.addName(" document.pushStackTraceInfo('" + sourceURI + "&"
                    + ts.getLineno() + "');\n ");
            consumeToken();
            decompiler.addToken(tt);
        ...
        case Token.NAME: {
            ...
            if (pn.getType() != Token.LABEL) {
                ...
                if (popFlag) {
                    decompiler.addName(" document.popStackTraceInfo(); ");
                }
                return pn;
            }
            break;
        ...
        if (popFlag) {
            decompiler.addName(" /* OUTERPOP */ document.popStackTraceInfo(); \n");
        }
        return pn;
    }
            ...
            ParserException
    {
        ...
        case Token.YIELD:
            ...
            break;
        default:
            decompiler.addName(" _zxy = ");
            e = expr(false);
            if (tt == Token.RETURN) {
                decompiler.addName("; document.popStackTraceInfo(); document.popFunctionInfo();\n /* FUNCTIONRETURN 1 (VALUE) */ return _zxy; alert('RETURN VALUE')");
                flag = true;
            }
            break;
        }
        if (!flag) {
            decompiler.addName("document.popStackTraceInfo();\n document.popFunctionInfo();\n /* FUNCTIONRETURN 2 (EMPTY) */ return; alert('RETURN EMPTY')");
        }
        int before = endFlags;
        Node ret;
            ...
            ParserException
    {
        ...
        if (tt == Token.NEW) {
             ...
             * "too many constructor arguments" - how many do we claim to
             * support?
             */
        ...
        tailLoop: for (;;) {
            ...
            case Token.DOTDOT: {
                ...
                case Token.NAME:
                    ...
                    if (s.equals("createElement")) // TODO
                    {
                        s = "createElement0";
                        flag = true;
                    }
                    else if (s.equals("appendChild")) {
                        // s = "appendChild0";
                        // domFlag = true;
                    }
    ...
    private Node primaryExpr() throws IOException, ParserException
    {
        ...
        case Token.NAME: {
            ...
            if (domFlag) {
                domFlag = false;
                // decompiler.addName(","+ts.getLineno());
            }
            if (compilerEnv.isXmlAvailable()) {
    ...
    private static String getInstrumentationCode() {
        InputStream rt = Parser.class.getResourceAsStream(Props.getProperty(SCRIPT_PROP));
        BufferedReader br = new BufferedReader(new InputStreamReader(rt));
        String code = "";
        String str = null;
        try {
    ...
    public static void main(String argv[]) {
        System.out.println("CODE: " + getInstrumentationCode());
    }
}
